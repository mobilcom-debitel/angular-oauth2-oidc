...
export class OAuthService extends AuthConfig {
  ...
  private createLoginUrl(
    ...
    return this.createAndSaveNonce().then((nonce: any) => {
      ...
      } else if (this.config.useNativeAppFlow) {
      ...
      return this.createAndSaveCodeChallenge().then( (challenge: string) => {
        ...
        if (this.config.useNativeAppFlow || this.config.usePKCE) {
          url += '&code_challenge=' + encodeURIComponent(challenge);
          url += '&code_challenge_method=' + this.config.codeChallengeMethod;
        }
  ...
  public tryLogin(options: LoginOptions = null): Promise<boolean> {
    ...
    let parts: object;
    let isAuthorizeCode = false;

    if (window.location.search && (window.location.search.startsWith('?code=') || window.location.search.includes('&code='))) {
      parts = this.urlHelper.parseQueryString(window.location.search.substring(1));
      isAuthorizeCode = true;
    } else if (options.customHashFragment) {
    ...
    parts['nonceInState'] = state;

    if (state) {
      const idx = state.indexOf(this.config.nonceStateSeparator);

      if (idx > -1) {
        parts['nonceInState'] = state.substr(0, idx);
    ...
    if (isAuthorizeCode) {
      return this.getTokenFromCode(options, parts);
    }

    return this.handleAuthorizationResponse(options, parts);

  }

  private handleAuthorizationResponse(options: LoginOptions = null, parts: object): Promise<boolean> {
    options = options || {};
    ...
    if (this.requestAccessToken && !options.disableOAuth2StateCheck && !parts['state']) {
      return Promise.resolve(false);
    }
    ...
    if (this.sessionChecksEnabled && !sessionState) {
      this.logger.warn(
        ...
        'were activated in the configuration but the authorization response ' +
        'does not contain a session_state parameter'
      );
    }
  ...
  public processIdToken(
    ...
    const header = jwt.decodePart(tokenParts[0]);
    const claims = jwt.decodePart(tokenParts[1]);
    const savedNonce = this._storage.getItem('nonce');
      ...
      .then(atHashValid => {
        ...
        return this.checkSignature(validationParams).then(_ => {
          const result: ParsedIdToken = {
            idToken: idToken,
            idTokenClaims: claims,
            idTokenClaimsJson: JSON.stringify(claims),
            idTokenHeader: header,
            idTokenHeaderJson: JSON.stringify(header),
   ...
   */
  private getTokenFromCode(options: LoginOptions = null, parts: object): Promise<boolean> {
    let params = new HttpParams()
    ...
    if (this.config.useNativeAppFlow || this.config.usePKCE) {
      const verifier = this._storage.getItem('code_verifier');
      params = params.set('code_verifier', verifier);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.http.post<TokenResponse>(this.tokenEndpoint, params, {headers}).subscribe(
        (tokenResponse) => {
          this.debug('code tokenResponse', tokenResponse);

          parts = {...parts, ...tokenResponse};
  ...
  public createAndSaveCodeChallenge(): Promise<string> {
    const that = this;
    return this.createNonce().then(function (verifier: any) {
      that._storage.setItem('code_verifier', verifier);
      if (that.config.codeChallengeMethod === 'S256') {
        return that.tokenValidationHandler.calcHash(verifier, 'SHA-256').then(challenge => {
          let challengeHash = btoa(challenge);
